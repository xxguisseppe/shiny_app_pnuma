...
umbral_dengue <- read.table("./DENGUE_v2.txt", header = T,sep = "\t")
umbral_malaria <- read.table("./MALARIA_v2.txt", header = T,sep = "\t")

ui <- navbarPage(title = tags$a(href ="/",
                                img(class = "logo", src = "www/LOGO_CDC.png",
                 ...
                 ############################# PANEL MALARIA FALCIPARUM - VIVAX #########################
                 navbarMenu(title = tags$h5(tags$b("MALARIA")),
                            tabPanel(title = tags$h6(tags$b(tags$em("FALCIPARUM"))),
                                     actionButton(inputId = "PISCO", label = "PISCO - DATASET"),
                                     actionButton(inputId = "ERA", label = "ERA5 - DATASET"),
                                     br(),
                                     fluidPage(br(),
                                               fluidRow(
                                                 column(width = 3,
...
server <- function(input, output, session){
  ...
  observeEvent(input$PISCO, {
    output$text <- renderUI({
      str1 <- umbral_dengue[umbral_dengue$ID %in% rv(),]$PISCO_MESS
  ...
  observeEvent(input$ERA, {
    output$text <- renderUI({
      str1 <- umbral_dengue[umbral_dengue$ID %in% rv(),]$ERA_MESS
      HTML(paste(str1, sep = '<br/>'))
    })
  })
  ######################## FILE "TXT MALARIA ###################################
  observeEvent(input$PISCO, {
    output$text2 <- renderUI({
      str1 <- umbral_malaria[umbral_malaria$ID %in% rv(),]$PISCO_FALCI
      HTML(paste(str1, sep = '<br/>'))
    })
  })
  ...
  observeEvent(input$ERA, {
    output$text2 <- renderUI({
      str1 <- umbral_malaria[umbral_malaria$ID %in% rv(),]$ERA_FALCI
      HTML(paste(str1, sep = '<br/>'))
    })
  })
